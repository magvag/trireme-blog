...
Подробнее о пункте 3 для тех, кто боится слова «Терминал»: откройте разархивированную папку `trireme-blog` и скопируйте полный путь до неё. В терминале наберите `cd`, через Ctrl+Shift+V вставьте путь, нажмите энтер и переходите к пункту 4.

![](newnote-2.png){.window} После п. 5 должно вылезти что-то такое
